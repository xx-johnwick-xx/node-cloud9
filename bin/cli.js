...
import Cloud9Client from '../src/Cloud9Client.js';
...
const clientToUse = settings.cliOptions?.clientToUse || settings.clientToUse || 'cloud9';
...
switch (clientToUse) {
    case 'bing':
        ...
        break;
    default:
        client = new Cloud9Client(
            settings.openaiApiKey || settings.cloud9Client.openaiApiKey,
            settings.cloud9Client,
            settings.cacheOptions,
        );
        break;
}

console.log(tryBoxen('Cloud9 CLI', {
...
async function onMessage(message) {
    ...
    switch (clientToUse) {
        case 'bing':
            ...
            break;
        default:
            aiLabel = settings.cloud9Client?.cloud9Label || 'Cloud9';
            break;
    }
    let reply = '';
...
async function deleteAllConversations() {
    if (clientToUse !== 'cloud9') {
        logWarning('Deleting all conversations is only supported for Cloud9 client.');
        return conversation();
    }
...
async function copyConversation() {
    if (clientToUse !== 'cloud9') {
        logWarning('Copying conversations is only supported for Cloud9 client.');
        return conversation();
    }
    if (!conversationData.conversationId) {
    ...
    const orderedMessages = Cloud9Client.getMessagesForConversation(messages, lastMessageId);
    const conversationString = orderedMessages.map(message => `#### ${message.role}:\n${message.message}`).join('\n\n');
    try {
        await clipboard.write(`${conversationString}\n\n----\nDeveloped by JohnWick#0001`);
