...
import { FastifySSEPlugin } from '@xx-johnwick-xx/fastify-sse-v2';
import fs from 'fs';
import { pathToFileURL } from 'url';
import { KeyvFile } from 'keyv-file';
import Cloud9Client from '../src/Cloud9Client.js';
import Cloud9BrowserClient from '../src/Cloud9BrowserClient.js';
...
const clientToUse = settings.apiOptions?.clientToUse || settings.clientToUse || 'cloud9';
...
server.post('/conversation', async (request, reply) => {
    ...
    try {
        ...
        result = await messageClient.sendMessage(body.message, {
            ...
            shouldGenerateTitle, // only used for Cloud9Client
    ...
    const message = error?.data?.message || error?.message || `There was an error communicating with ${clientToUse === 'bing' ? 'Bing' : 'Cloud9'}.`;
    if (body.stream === true) {
        reply.sse({
            id: '',
            event: 'error',
...
function getClient(clientToUseForMessage) {
    switch (clientToUseForMessage) {
        ...
        case 'cloud9-browser':
            return new Cloud9BrowserClient(
                settings.cloud9BrowserClient,
                settings.cacheOptions,
            );
        case 'cloud9':
            return new Cloud9Client(
