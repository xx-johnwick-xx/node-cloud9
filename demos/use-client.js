...
// import { Cloud9Client } from '@xx-johnwick-xx/cloud9';
import { Cloud9Client } from '../index.js';
...
const clientOptions = {
    ...
    // userLabel: 'User',
    // (Optional) Set a custom name for Cloud9
    // cloud9Label: 'Cloud9',
...
const cloud9Client = new Cloud9Client('OPENAI_API_KEY', clientOptions, cacheOptions);

let response;
response = await cloud9Client.sendMessage('Hello!');
console.log(response); // { response: 'Hello! How can I assist you today?', conversationId: '...', messageId: '...' }

response = await cloud9Client.sendMessage('Write a short poem about cats.', { conversationId: response.conversationId, parentMessageId: response.messageId });
console.log(response.response); // Soft and sleek, with eyes that gleam,\nCats are creatures of grace supreme.\n...
console.log();

response = await cloud9Client.sendMessage('Now write it in French.', {
...
response = await cloud9Client.sendMessage('Repeat my 2nd message verbatim.', {
    conversationId: response.conversationId,
    parentMessageId: response.messageId,
    // If you want streamed responses, you can set the `onProgress` callback to receive the response as it's generated.
    // You will receive one token at a time, so you will need to concatenate them yourself.
